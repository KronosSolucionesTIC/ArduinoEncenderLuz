...
IPAddress subnet(255, 255, 255, 0);                      //Asignamos la mascara de subred a la Ethernet Shield
 
int led = 8;             //Definimos el pin que usaremos para encender el led
    ...
    while (client.connected()) {                       //Si el cliente esta conectado...
      if (client.available()) {                        //Y hay datos para recibir...
        ...
        if (c == '\n' && currentLineIsBlank) {
            ...
            client.println("<title>Kronos Soluciones TIC</title>");
            client.println("</head>");
            client.println("");
            client.println("<body>");
            client.println("<center><img src='http://www.globater.com/kronos.jpg'></center>");
